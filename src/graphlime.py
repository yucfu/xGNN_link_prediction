...
class GraphLIME:
    ...
    def __subgraph__lp(self, source_node, target_node, x, y, edge_index, **kwargs):
        num_nodes, num_edges = x.size(0), edge_index.size(1)

        subset, edge_index, mapping, edge_mask = k_hop_subgraph(
            [source_node, target_node], self.hop, edge_index,
            relabel_nodes=True, num_nodes=num_nodes, flow=self.__flow__())
    ...
    def __init_predict__lp(self, x, edge_index, edge_label_index, **kwargs):
        if self.cached and self.cached_result is not None:
            if x.size(0) != self.cached_result.size(0):
                raise RuntimeError(
                    'Cached {} number of nodes, but found {}.'.format(
        ...
        if not self.cached or self.cached_result is None:
            with torch.no_grad():
                log_logits = self.model(x=x, edge_index=edge_index,
                                        edge_label_index=edge_label_index, **kwargs)
                # probas = log_logits.exp()

            self.cached_result = log_logits
    ...
    def explain_node(self, node_idx, x, edge_index, **kwargs):
        ...
        # x: feature matrix of subgraph: [31, 1433]

        x = x.detach().cpu().numpy()  # (n, d)
        y = probas.detach().cpu().numpy()  # (n, classes)

        n, d = x.shape  # n: num of neighbors, d: num of features
    ...
    def explain_link(self, x, edge_index, edge_label_index, **kwargs):

        source_node, target_node = edge_label_index.numpy()[:, 0]
        probas = self.__init_predict__lp(x, edge_index, edge_label_index, **kwargs)
        print()

        x, probas, _, _, _, _ = self.__subgraph__lp(
            source_node, target_node, x, probas, edge_index, **kwargs)
